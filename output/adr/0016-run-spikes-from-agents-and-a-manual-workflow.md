...
- **Credentials.** One Desktop OAuth client in a standard Cloud project owned by the test account. `auth.mjs` runs a loopback flow with PKCE, once, and saves the refresh token to `.env` without printing it. The token's scopes are the four manifest scopes plus:
  ...
  - `script.deployments`, so that if `scripts.run` turned out to need an API-executable deployment even in `devMode`, the runner could create one without a second consent round. Verification (2026-09-26, `spikes/00-profile.md`) showed it isn't needed: the manifest's `executionApi` key gives the HEAD deployment an `EXECUTION_API` entry point, and `devMode` on the script ID works. The scope is now used only by `check` (listing deployments) and the `deploy` fallback; drop it the next time the token is minted.
