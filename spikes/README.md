...
| [19-message-added](19-message-added.md) | #19 | Run 2026-09-26 (filter, S08c, and S15 cases pending). Record `labelIds` are the labels when the message was added; each message has one `messageAdded` record; import ignores `threadId` and sets `internalDate` from the `Date` header. |
