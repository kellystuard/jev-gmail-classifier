...
- **Jev request:** `POST https://api.typesafe.ai/v1/systemone` with `Authorization: Bearer <key>`.
  One request per thread (the API accepts one `state` per request), with **all** rules sent together as Noul (yes/no probability) questions.
  Requests within a run go out concurrently via `UrlFetchApp.fetchAll`.
